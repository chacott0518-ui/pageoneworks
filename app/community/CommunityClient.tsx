...
const CAT_STYLE: Record<string, string> = {
  free: 'bg-zinc-500/15 text-zinc-300', humor: 'bg-yellow-500/15 text-yellow-300',
  politics: 'bg-rose-500/15 text-rose-300', buy: 'bg-orange-500/15 text-orange-300',
  estate: 'bg-emerald-500/15 text-emerald-300', stock: 'bg-cyan-500/15 text-cyan-300',
  insurance: 'bg-indigo-500/15 text-indigo-300', biz: 'bg-violet-500/15 text-violet-300',
  law: 'bg-purple-500/15 text-purple-300', health: 'bg-red-500/15 text-red-300',
  mental: 'bg-pink-500/15 text-pink-300', silver: 'bg-slate-500/15 text-slate-300',
  diet: 'bg-lime-500/15 text-lime-300', it: 'bg-sky-500/15 text-sky-300',
  game: 'bg-blue-500/15 text-blue-300', auto: 'bg-blue-600/15 text-blue-300',
  golf: 'bg-green-500/15 text-green-300', beauty: 'bg-pink-400/15 text-pink-300',
  food: 'bg-amber-500/15 text-amber-300', pet: 'bg-teal-500/15 text-teal-300',
  edu: 'bg-teal-600/15 text-teal-300', ad: 'bg-gold/15 text-gold',
  anon: 'bg-gray-500/15 text-gray-400', '': 'bg-white/8 text-white/50',
}
...
const DUMMY_POSTS: Post[] = [
  {
    ...
    content: '3년 만에 드디어 청약 당첨됐습니다. 가점제로 넣었는데 이번에 운이 좋았네요.',
  ...
  },
  {
    ...
    content: '간수치가 조금 높게 나왔는데 걱정이 되네요.',
  ...
  },
  {
    ...
    content: '내추럴 와인 종류가 엄청 다양하고 안주도 훌륭합니다.',
  ...
  },
  {
    ...
    content: '최소 20명 모이면 진행합니다. 현재 14명 참여 중.',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md px-4">
      <div className="w-full max-w-sm bg-[#18181B] border border-white/10 rounded-2xl overflow-hidden"
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-0">
          <div />
          <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full text-white/30 hover:text-white/60 hover:bg-white/5 transition-all">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="px-8 pb-8">
          <div className="text-center mb-6">
            <p className="text-[11px] tracking-[0.25em] uppercase text-gold mb-1" style={{ fontFamily: 'var(--font-space-mono)' }}>PAGEONEWORKS</p>
            <p className="text-[10px] text-white/30 tracking-widest">Premium Community</p>
          </div>
          <h2 className="text-[18px] font-extrabold text-white text-center mb-1 tracking-tight">로그인 후 이용하세요</h2>
          <p className="text-[13px] text-white/40 text-center mb-6 leading-relaxed">글 작성, 댓글, 좋아요는 로그인 후 이용 가능합니다</p>
          <div className="bg-gold/8 border border-gold/20 rounded-lg px-4 py-3 mb-5 text-center">
            <p className="text-[12px] text-gold/80">🔒 소셜 계정 외 별도 정보를 저장하지 않습니다</p>
          </div>
          ...
          <p className="text-[11px] text-white/20 text-center">
            로그인 시 <span className="underline cursor-pointer hover:text-white/40">이용약관</span> 및 <span className="underline cursor-pointer hover:text-white/40">개인정보처리방침</span>에 동의하는 것으로 간주됩니다
          </p>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/80 backdrop-blur-md">
      <div className="w-full max-w-2xl bg-[#18181B] border border-white/10 md:rounded-2xl shadow-2xl">
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          <textarea placeholder="내용을 입력하세요. @멘션, #해시태그 사용 가능" value={content} onChange={e => setContent(e.target.value)}
            rows={7} className="bg-white/5 border border-white/10 text-white/80 text-[13px] px-4 py-3 rounded-xl placeholder:text-white/25 focus:outline-none focus:border-gold/40 transition-colors resize-none leading-relaxed" />
          <input type="text" placeholder="#태그 입력 (쉼표로 구분, 최대 5개)" value={tags} onChange={e => setTags(e.target.value)}
...
function PostCard({ post, onLikeToggle }: { post: Post; onLikeToggle: (id: string) => void }) {
  ...
  return (
    <Link href={`/community/${post.id}`} className="block">
      <article
        className="group bg-[#1E1E22] border border-white/[0.10] rounded-xl overflow-hidden transition-all duration-200 hover:-translate-y-[2px] hover:border-white/[0.18] relative"
        style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.3)' }}
        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.boxShadow = '0 8px 24px rgba(0,0,0,0.4), 0 2px 8px rgba(201,169,110,0.06)' }}
        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.boxShadow = '0 1px 3px rgba(0,0,0,0.3)' }}
      >
        ...
        <div className="p-4 md:p-5 relative">
          <div className="flex items-center gap-2.5 mb-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${catStyle}`}>{post.category_slug}</span>
                <span className="text-[11px] text-[#5A5450]">{timeAgo(post.created_at)}</span>
              </div>
            </div>
            <div className="flex items-center gap-1.5">
              {post.is_hot && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-[3px]" style={{ background: 'rgba(232,93,74,0.15)', color: '#F07060' }}>HOT</span>}
              {post.is_best && <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-[3px]" style={{ background: 'rgba(74,158,232,0.15)', color: '#70B0F0' }}>BEST</span>}
            </div>
            <div className="relative">
              <button onClick={e => { e.preventDefault(); e.stopPropagation(); setMenuOpen(!menuOpen) }}
          ...
          <div className="flex gap-3">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[12px] text-[#7A7268] leading-relaxed line-clamp-1 mb-2.5">{post.content}</p>
              <div className="flex flex-wrap gap-1.5">
                {post.tags.map(tag => (
                  <span key={tag} className="text-[10px] text-[#4A4640] hover:text-gold/60 transition-colors cursor-pointer">#{tag}</span>
                ))}
              </div>
            </div>
            <div className="w-[68px] h-[56px] rounded-lg shrink-0 flex items-center justify-center text-2xl border border-white/8 overflow-hidden transition-transform duration-200 group-hover:scale-[1.03]"
              style={{ background: gradient }}>
              {!hasThumb && <span style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.4))' }}>{emoji}</span>}
            </div>
          </div>

          <div className="flex items-center gap-3 pt-3 mt-3 border-t border-white/[0.07]">
            <button onClick={e => { e.preventDefault(); e.stopPropagation(); setLiked(!liked); onLikeToggle(post.id) }}
            ...
            <button onClick={e => { e.preventDefault(); e.stopPropagation() }}
              className="flex items-center gap-1 text-[12px] text-[#4A4640] hover:text-[#8A8278] transition-colors">
...
export default function CommunityClient() {
  ...
  return (
    <>
      ...
      {writeOpen && <WriteModal onClose={() => setWriteOpen(false)} onLoginRequired={() => setLoginOpen(true)} />}
      ...
      <main className="min-h-screen bg-[#0F0F10] pt-[60px]">
        <div className="border-b border-white/[0.06] px-6 md:px-10 py-8 bg-[#141416]">
          <div className="max-w-[1440px] mx-auto flex items-end justify-between">
            <div>
              ...
              <p className="text-[13px] text-[#5A5450] hidden md:block">검증된 전문가들과 함께하는 인사이트 커뮤니티</p>
              <div className="flex gap-6 mt-4">
                {[{ n: '2,847', l: '전체 글' }, { n: '3,847', l: '회원' }, { n: '127', l: '오늘 글' }, { n: '892', l: '오늘 방문' }].map(s => (
                  <div key={s.l}>
                    <div className="text-[20px] font-black text-[#F0E8D8]" style={{ fontVariantNumeric: 'tabular-nums', letterSpacing: '-0.04em' }}>{s.n}</div>
          ...
          {searchOpen && (
            <div className="max-w-[1440px] mx-auto mt-4">
              <input type="search" autoFocus placeholder="제목, 본문, 태그로 검색" value={searchQuery}
            ...
            {CATEGORIES.map(cat => (
              <button key={cat.slug} onClick={() => setActiveCategory(cat.slug)}
                className={`shrink-0 flex items-center gap-1.5 text-[12px] px-4 py-3.5 border-b-2 transition-all whitespace-nowrap font-medium ${
                  activeCategory === cat.slug ? 'border-gold text-gold' : 'border-transparent text-[#4A4640] hover:text-[#8A8278]'
                }`}>
                {cat.label}
                <span className="text-[10px] px-1.5 py-0.5 rounded-full"
                  style={{ background: activeCategory === cat.slug ? 'rgba(201,169,110,0.15)' : 'rgba(255,255,255,0.04)', color: activeCategory === cat.slug ? '#C9A96E' : '#3A3630' }}>
                  {cat.count}
                </span>
              </button>
            ))}
        ...
        <div className="max-w-[1440px] mx-auto px-6 md:px-10">
          <div className="grid grid-cols-1 md:grid-cols-[200px_1fr_244px]">
            ...
            <aside className="hidden md:block border-r border-white/[0.06] pr-5 pt-5">
              <p className="text-[9px] tracking-[0.2em] uppercase text-[#3A3630] mb-3 px-2" style={{ fontFamily: 'var(--font-space-mono)' }}>카테고리</p>
              <nav className="flex flex-col">
                {CATEGORIES.map(cat => (
                  <button key={cat.slug} onClick={() => setActiveCategory(cat.slug)}
                    className={`flex items-center justify-between text-left text-[13px] px-3 py-2 rounded-lg transition-all ${
                      activeCategory === cat.slug ? 'font-bold text-gold' : 'text-[#5A5450] hover:text-[#A09080] hover:bg-white/3'
                    }`}
                    style={activeCategory === cat.slug ? { background: 'rgba(201,169,110,0.08)' } : {}}>
                    {cat.label}
                    <span className={`text-[10px] ${activeCategory === cat.slug ? 'text-gold/60' : 'text-[#3A3630]'}`}>{cat.count}</span>
                  </button>
                ))}
              </nav>
            ...
            <section className="py-4 px-0 md:px-5 border-r border-white/[0.06]">
                ...
                {[
                  ...
                  <button key={key} onClick={() => setSortBy(key as typeof sortBy)}
                    className={`flex items-center gap-1 text-[11px] font-semibold px-3 py-1.5 rounded-full border transition-all ${
                      sortBy === key ? 'border-gold/40 text-gold' : 'border-white/8 text-[#4A4640] hover:text-[#7A7268]'
                    }`}
              ...
              {NOTICES.map((n, i) => (
                <div key={i} className="flex items-center gap-2.5 border rounded-lg px-4 py-2.5 mb-2 cursor-pointer transition-colors"
            ...
            <aside className="hidden md:block pl-5 pt-5">
                ...
                <div className="flex flex-col">
                  {TRENDING.map((item, i) => (
                    <Link key={item.id} href={`/community/${item.id}`}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] text-[#5A5450] group-hover:text-[#A09080] transition-colors leading-snug line-clamp-2">{item.title}</p>
                        <div className="flex gap-2 mt-1">
                          <span className="text-[10px] text-[#3A3630] flex items-center gap-0.5"><MessageCircle className="w-2.5 h-2.5" /> {item.comment_count}</span>
                          <span className="text-[10px] text-[#3A3630] flex items-center gap-0.5"><Eye className="w-2.5 h-2.5" /> {item.view_count.toLocaleString()}</span>
                        </div>
                      </div>
                    </Link>
                  ))}
              ...
              <div className="mb-5">
                ...
                <div className="flex flex-col">
                  {[{ l: '새 게시글', v: '127' }, { l: '새 댓글', v: '384' }, { l: '방문자', v: '892' }, { l: '신규 가입', v: '14' }].map(s => (
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 border-t border-white/[0.07] grid grid-cols-5 z-30"
          ...
          {[
            ...
            item.icon === null ? (
              <button key={i} onClick={handleWriteClick} className="flex flex-col items-center justify-center py-1 relative">
                <div className="w-10 h-10 bg-gold rounded-full flex items-center justify-center -mt-6"
              ...
              <a key={i} href={item.href!} className="flex flex-col items-center justify-center py-2.5 gap-0.5 text-[#4A4640] hover:text-[#8A8278] transition-colors">
